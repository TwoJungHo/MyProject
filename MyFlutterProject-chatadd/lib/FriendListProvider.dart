...
import 'package:myflutterproject/MyProfile.dart';
...
class FriendListProvider with ChangeNotifier {
  ...
  Future<bool> _deleteFriend(String friendname) async {
    ...
    if (friendname.isNotEmpty) {
      ...
      final deleteResponse = await http.delete(
        Uri.parse("http://10.0.2.2:9000/friend/deletefriend/$friendname"),
  ...
  void addFriend(String friendname) async {
    if (friendname.isNotEmpty) {
      ...
      if (addResponse.statusCode == 200) {
        ...
        _fetchFriendList(); // 친추 성공시 친구 목록 업데이트
  ...
  // 친구 이미지 불러오기
  Future<MemoryImage?> fetchFriendImage(String? friendname) async {
    final response = await http
